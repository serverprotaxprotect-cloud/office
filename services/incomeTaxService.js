...
async function logIncomeTax(conn, payload) {
  await conn.query(
    `INSERT INTO income_tax_history_log
      (income_tax_client_id, filing_id, action, old_value, new_value, remarks, updated_by_id, updated_by_name)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [
      payload.income_tax_client_id || null,
...
async function createTaskForFiling(conn, taxClient, filing, actor) {
  if (!filing.assigned_to_id) return null;
  const client = await findClient(conn, taxClient.client_id);
  ...
  const assigneeName = filing.assigned_to_name || taxClient.default_assignee_name || filing.assigned_to_id;
  ...
  await conn.query(
    ...
    [
      ...
      assigneeName,
      taxClient.client_id,
      client?.agent_id || taxClient.agent_id || null,
      client?.agent_name || taxClient.agent_name || null,
      client?.legal_name || taxClient.taxpayer_name || null,
      client?.business_name || taxClient.reference_client_name || null,
      taxClient.contact_number || client?.mobile_number || null,
  ...
  await conn.query(
    ...
    [
      ...
      createdById,
      createdByName,
      'Created from Income Tax filing tracker',
    ]
  );
  return taskId;
}

function taskStatusForITR(status) {
...
function itrStatusForTask(status) {
  if (status === 'Completed') return 'Filed';
  if (status === 'Waiting for Client') return 'Pending by Client';
  if (['Pending', 'In Progress', 'Waiting for Government', 'Under Review', 'On Hold', 'Reassigned'].includes(status)) return 'Pending';
...
async function syncTaskForFiling(conn, filing, status, actor, remark) {
  ...
  const taskStatus = taskStatusForITR(status);
  ...
  await conn.query(
    `INSERT INTO task_history
     ...
     VALUES ($1,$2,'ITR Status Sync',$3,$4,$5,$6,$7,$8,$9,$10,NOW(),$11)`,
    [
      `LOG_${uuidv4().replace(/-/g, '').slice(0, 10)}`,
      ...
      actorId(actor),
      actorName(actor),
      remark || `Income Tax filing marked ${status}`,
    ]
  );
}

async function syncIncomeTaxForTaskStatus(conn, task, status, actor, remark) {
  const itrStatus = itrStatusForTask(status);
  if (!itrStatus) return null;
  let filingRes;
  try {
    filingRes = await conn.query(
      `SELECT * FROM income_tax_filing_records WHERE linked_task_id=$1 FOR UPDATE`,
      [task.task_id]
    );
  } catch (err) {
  ...
  const admin = isIncomeTaxAdmin(actor);
  const assigned = filing.assigned_to_id === actor.emp_id || task.assigned_to_id === actor.emp_id;
  if (!admin && !assigned) {
    const err = new Error('Only assigned employee or admin can update linked Income Tax filing status');
    err.statusCode = 403;
    throw err;
  }
  if (!admin && filing.status === 'Filed' && dateOnly(filing.filed_date_ist) && dateOnly(filing.filed_date_ist) < todayIST() && itrStatus !== 'Filed') {
    const err = new Error('Filed Income Tax record is locked. Admin can reopen it.');
    err.statusCode = 403;
    throw err;
  }
  if (filing.status === itrStatus) return { filing_id: filing.id, status: itrStatus, changed: false };
  const filedDate = itrStatus === 'Filed' ? todayIST() : null;
  const filedAtSql = itrStatus === 'Filed' ? 'NOW()' : 'NULL';
  await conn.query(
    `UPDATE income_tax_filing_records SET
       status=$1,
       filed_date_ist=$2,
       filed_at=${filedAtSql},
       ...
       updated_at=NOW()
     WHERE id=$5`,
    [itrStatus, filedDate, actorId(actor), actorName(actor), filing.id]
  );
  await logIncomeTax(conn, {
    income_tax_client_id: filing.income_tax_client_id,
    filing_id: filing.id,
    action: filing.status === 'Filed' && itrStatus !== 'Filed' ? (admin ? 'AdminTaskRewriteStatus' : 'TaskRewriteFiledSameDay') : 'TaskStatusSync',
    old_value: { status: filing.status, task_status: task.status },
    new_value: { status: itrStatus, task_status: status },
    remarks: remark || `Task marked ${status}`,
    actor,
  });
  return { filing_id: filing.id, status: itrStatus, changed: true };
}

async function createOrAttachFiling(conn, taxClient, assessmentYear, actor, dueDate, source = 'manual') {
  const assignedToId = taxClient.default_assignee_id || null;
  const assignedToName = taxClient.default_assignee_name || null;
  const fy = financialYearForAssessmentYear(assessmentYear);
  const resolvedDueDate = dueDate || dueDateForAssessmentYear(assessmentYear);
  let filingRes = await conn.query(
    `INSERT INTO income_tax_filing_records
     ...
     VALUES ($1,$2,$3,$4,$5,$6,$7,NULL,$8,$9,'Not Started',$10,$11,$12)
     ON CONFLICT (organization_id, income_tax_client_id, assessment_year) DO NOTHING
     RETURNING *`,
    [
      taxClient.id,
      taxClient.client_id,
      taxClient.taxpayer_name,
      normalizePan(taxClient.pan_number),
      fy,
      assessmentYear,
      resolvedDueDate,
      assignedToId,
  ...
  if (!filingRes.rows.length) {
    created = false;
    filingRes = await conn.query(
      `SELECT * FROM income_tax_filing_records WHERE income_tax_client_id=$1 AND assessment_year=$2`,
      [taxClient.id, assessmentYear]
    );
  }
  const filing = filingRes.rows[0];
  ...
  if (!taskId && !['Filed', 'Not Applicable'].includes(filing.status) && filing.assigned_to_id) {
    taskId = await createTaskForFiling(conn, taxClient, filing, actor);
    if (taskId) {
      await conn.query(`UPDATE income_tax_filing_records SET linked_task_id=$1, updated_at=NOW() WHERE id=$2`, [taskId, filing.id]);
    }
  }
  if (created) {
    await logIncomeTax(conn, {
      income_tax_client_id: taxClient.id,
      filing_id: filing.id,
      action: 'GenerateFiling',
      new_value: { assessment_year: assessmentYear, task_id: taskId },
      actor,
    });
  }
...
async function generateFilingsForYear({ assessmentYear, dueDate, actor, source = 'manual' }) {
  const conn = await db.pool.connect();
  const summary = { assessment_year: assessmentYear, clients_seen: 0, filings_created: 0, tasks_created: 0, existing: 0, skipped: 0 };
  try {
    ...
    const clients = await conn.query(
      `SELECT * FROM income_tax_clients
       ...
       ORDER BY taxpayer_name, id`,
      [dueDate || dueDateForAssessmentYear(assessmentYear)]
    );
    summary.clients_seen = clients.rows.length;
    for (const taxClient of clients.rows) {
      const result = await createOrAttachFiling(conn, taxClient, assessmentYear, actor, dueDate, source);
...
async function updateFilingStatus(filingId, { status, itr_type, due_date, remark }, actor) {
  if (status && !ITR_STATUSES.includes(status)) {
    const err = new Error('Invalid Income Tax status');
    err.statusCode = 400;
    throw err;
  }
  const conn = await db.pool.connect();
  try {
    await conn.query('BEGIN');
    const r = await conn.query('SELECT * FROM income_tax_filing_records WHERE id=$1 FOR UPDATE', [filingId]);
    if (!r.rows.length) {
      const err = new Error('Income Tax filing not found');
      err.statusCode = 404;
      throw err;
    }
    ...
    if (!admin && !assigned) {
      const err = new Error('Only assigned employee or admin can update this Income Tax filing');
      err.statusCode = 403;
      throw err;
    }
    ...
    const filedDate = nextStatus === 'Filed' ? todayIST() : null;
    const filedAtSql = nextStatus === 'Filed' ? 'NOW()' : 'NULL';
    await conn.query(
      `UPDATE income_tax_filing_records SET
         status=$1,
         itr_type=COALESCE(NULLIF($2,''), itr_type),
         ...
         updated_at=NOW()
       WHERE id=$7`,
      [nextStatus, itr_type || '', due_date || null, filedDate, actorId(actor), actorName(actor), filingId]
    );
    const updated = { ...filing, status: nextStatus, itr_type: itr_type || filing.itr_type, due_date: due_date || filing.due_date };
    await logIncomeTax(conn, {
      income_tax_client_id: filing.income_tax_client_id,
      filing_id: filing.id,
      action: filing.status === 'Filed' && nextStatus !== 'Filed' ? (admin ? 'AdminRewriteStatus' : 'RewriteFiledSameDay') : 'UpdateStatus',
      old_value: { status: filing.status, itr_type: filing.itr_type, due_date: filing.due_date },
      new_value: { status: nextStatus, itr_type, due_date },
      remarks: remark,
      actor,
    });
    await syncTaskForFiling(conn, updated, nextStatus, actor, remark);
    await conn.query('COMMIT');
    return { success: true };
  } catch (err) {
    await conn.query('ROLLBACK');
...
async function assignFiling(filingId, assigneeId, remark, actor) {
  ...
  try {
    ...
    if (!admin && !assigned && !unassigned) {
      const err = new Error('Only assigned employee or admin can reassign this Income Tax filing');
      err.statusCode = 403;
      throw err;
    }
    const assigneeName = employee.formal_name || employee.name;
    await conn.query(
      `UPDATE income_tax_filing_records SET assigned_to_id=$1, assigned_to_name=$2, updated_at=NOW() WHERE id=$3`,
    ...
    if (filing.linked_task_id) {
      ...
      await conn.query(
        `INSERT INTO task_history
          (log_id, task_id, action, old_assigned_to, new_assigned_to, old_due_date, new_due_date,
           updated_by_id, updated_by_name, updated_at, remark)
         VALUES ($1,$2,'ITR Reassigned',$3,$4,$5,$6,$7,$8,NOW(),$9)`,
        [
          `LOG_${uuidv4().replace(/-/g, '').slice(0, 10)}`,
          filing.linked_task_id,
          ...
          actorId(actor),
          actorName(actor),
          remark || 'Income Tax filing reassigned',
        ]
      );
    } else if (!['Filed', 'Not Applicable'].includes(filing.status)) {
      const client = await conn.query('SELECT * FROM income_tax_clients WHERE id=$1', [filing.income_tax_client_id]);
      ...
      if (taskId) await conn.query('UPDATE income_tax_filing_records SET linked_task_id=$1 WHERE id=$2', [taskId, filingId]);
    }
    await logIncomeTax(conn, {
      income_tax_client_id: filing.income_tax_client_id,
      filing_id: filing.id,
    ...
    await conn.query('COMMIT');
    return { assigned_to_id: employee.emp_id, assigned_to_name: assigneeName };
  } catch (err) {
    await conn.query('ROLLBACK');
    throw err;
  } finally {
...
async function assignUnassignedFilingsForClient(conn, incomeTaxClientId, employee, actor, options = {}) {
  ...
  if (options.assessmentYear) {
    const assessmentYear = options.assessmentYear;
    const created = await conn.query(
      ...
      [
        ...
        taxClient.taxpayer_name,
        normalizePan(taxClient.pan_number),
        financialYearForAssessmentYear(assessmentYear),
        assessmentYear,
        options.dueDate || dueDateForAssessmentYear(assessmentYear),
        employee.emp_id,
        assigneeName,
    ...
    if (created.rows.length) {
      ...
      const existing = await conn.query(
        `SELECT ifr.*, t.task_id AS active_task_id
         FROM income_tax_filing_records ifr
         LEFT JOIN tasks t ON t.task_id=ifr.linked_task_id AND t.active_flag=true
         WHERE ifr.income_tax_client_id=$1 AND ifr.assessment_year=$2
         FOR UPDATE OF ifr`,
        [incomeTaxClientId, assessmentYear]
      );
    ...
    const existing = await conn.query(
      `SELECT ifr.*, t.task_id AS active_task_id
       FROM income_tax_filing_records ifr
       LEFT JOIN tasks t ON t.task_id=ifr.linked_task_id AND t.active_flag=true
       WHERE ifr.income_tax_client_id=$1
         AND ifr.status NOT IN ('Filed','Not Applicable')
         AND (COALESCE(ifr.assigned_to_id,'')='' OR ifr.linked_task_id IS NULL OR t.task_id IS NULL)
       ORDER BY ifr.assessment_year
       FOR UPDATE OF ifr`,
      [incomeTaxClientId]
    );
  ...
  for (const filing of filings) {
    ...
    await conn.query(
      `UPDATE income_tax_filing_records
    ...
    if (filing.linked_task_id && filing.active_task_id) {
      ...
      const updatedFiling = { ...filing, assigned_to_id: employee.emp_id, assigned_to_name: assigneeName };
      const taskId = await createTaskForFiling(conn, taxClient, updatedFiling, actor);
      if (taskId) {
        tasksCreated += 1;
        await conn.query(
          'UPDATE income_tax_filing_records SET linked_task_id=$1, updated_at=NOW() WHERE id=$2',
          [taskId, filing.id]
        );
      }
    }
    ...
    await logIncomeTax(conn, {
      ...
      new_value: { assigned_to_id: employee.emp_id, assigned_to_name: assigneeName },
      remarks: options.remark || 'Assigned from unassigned Income Tax client',
      actor,
    });
  }
