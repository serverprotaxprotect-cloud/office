...
// ── GET /api/tasks/meta  (work names, statuses, priorities, employees) ──
...
router.get('/', authMiddleware, async (req, res) => {
  const { emp_id } = req.user;
  const { view = 'my', status, priority, work_name, search, page = 1, limit = 50 } = req.query;
  ...
  if (view === 'my') {
    // My Tasks = assigned to me by someone ELSE (exclude self-assigned)
    ...
    // Assigned by Me = tasks I created and assigned to OTHERS only
  ...
  // view === 'all' → no emp filter (all tasks visible)
  ...
  if (search) {
    ...
    conds.push(`(t.task_id ILIKE $${n} OR t.legal_name ILIKE $${n} OR t.business_name ILIKE $${n} OR t.work_name ILIKE $${n} OR t.client_id ILIKE $${n})`);
  }
...
router.get('/notifications', authMiddleware, async (req, res) => {
  const { emp_id } = req.user;
  try {
    // Today due tasks (assigned to me)
    ...
    // Recently assigned to me (last 7 days, by others)
    const assignedRes = await db.query(
      `SELECT task_id, work_name, legal_name, business_name, client_id, due_date, status, priority,
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  const {
    ...
    internal_remark, professional_fees, challan_amount, other_expense,
    fees_applicable,
  } = req.body;
  ...
  // Count today's tasks for sequential ID
  try {
    ...
    await db.query(
      `INSERT INTO tasks (
        ...
        work_name, work_description, priority, status, due_date,
        internal_remark, fees_applicable, challan_amount, professional_fees, other_expense, total_amount,
        ...
        $15,$16,$17,'Pending',$18,
        $19,$20,$21,$22,$23,$24,
        $25,'Not Applicable',true
      )`,
      [
        taskId, now, emp_id, formal_name || name, toId, toName,
        client_id || null, agent_id || null, agent_name || null, legal_name || null, business_name || null,
        mobile_number || null, email_id || null, drive_link || null,
        work_name, work_description || null, priority || 'Medium', due_date || null,
        internal_remark || null, fees_applicable || null,
        parseFloat(challan_amount) || null, parseFloat(professional_fees) || null,
        parseFloat(other_expense) || null, total || null,
        isSelf,
      ]
    );
    ...
    await db.query(
      ...
      [
        ...
        taskId, toName, due_date || null, emp_id, formal_name || name,
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  const {
    status, priority, due_date, assigned_to_id, assigned_to_name,
    internal_remark, client_pending_remark, completion_remark,
    next_followup_date, drive_link,
    professional_fees, challan_amount, other_expense, fees_applicable,
  } = req.body;
  ...
  try {
    ...
    const total = (parseFloat(professional_fees) ?? old.professional_fees ?? 0) +
                  (parseFloat(challan_amount) ?? old.challan_amount ?? 0) +
                  (parseFloat(other_expense) ?? old.other_expense ?? 0);

    const completionDate = status && ['Completed', 'Cancelled'].includes(status) && !old.completion_date
      ? new Date().toISOString().split('T')[0] : old.completion_date;
    ...
    await db.query(
      `UPDATE tasks SET
        ...
        internal_remark = COALESCE($6, internal_remark),
        client_pending_remark = COALESCE($7, client_pending_remark),
        completion_remark = COALESCE($8, completion_remark),
        next_followup_date = COALESCE($9::date, next_followup_date),
        drive_link = COALESCE($10, drive_link),
        professional_fees = COALESCE($11, professional_fees),
        challan_amount = COALESCE($12, challan_amount),
        other_expense = COALESCE($13, other_expense),
        total_amount = $14,
        fees_applicable = COALESCE($15, fees_applicable),
        completion_date = $16,
        last_updated_at = NOW(),
        last_updated_by_id = $17,
        last_updated_by_name = $18
       WHERE task_id = $19`,
      [
        status || null, priority || null, due_date || null,
        assigned_to_id || null, assigned_to_name || null,
        internal_remark || null, client_pending_remark || null, completion_remark || null,
        next_followup_date || null, drive_link || null,
        parseFloat(professional_fees) || null, parseFloat(challan_amount) || null,
        parseFloat(other_expense) || null, total || null,
        fees_applicable || null, completionDate || null,
        emp_id, formal_name || name, taskId,
      ]
    );
    ...
    await db.query(
      ...
      [
        ...
        old.assigned_to_name, assigned_to_name || old.assigned_to_name,
        old.due_date, due_date || old.due_date,
        emp_id, formal_name || name,
        internal_remark || completion_remark || null,
      ]
    );
